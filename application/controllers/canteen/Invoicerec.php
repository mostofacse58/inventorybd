...
class Invoicerec extends My_Controller {
    function __construct(){
        parent::__construct();
        $this->load->model('shipping/Import_model');
        $this->load->model('canteen/Invoice_model');
        $this->load->model('canteen/Invoicerec_model');
    ...
    function lists(){
        ...
        $config['base_url']=base_url().'canteen/Invoicerec/lists/';
        $config['suffix'] = '?' . http_build_query($_GET, '', "&");
        $config["uri_segment"] = 4;
        $config['total_rows'] = $this->Invoicerec_model->get_count();
        ...
        $data['list']=$this->Invoicerec_model->lists($config["per_page"],$data['page']);
        ...
        $data['display']='canteen/Invoicereclist';
  ...
  function checkform($invoice_id=FALSE){
    ...
    $data['info']=$this->Invoice_model->get_info($invoice_id);
    $data['detail']=$this->Invoice_model->getDetails($invoice_id);
    $data['display']='canteen/invoiceReceivedview';
    $this->load->view('admin/master',$data);
  }
  function updated($invoice_id=FALSE){
    $check=$this->Invoicerec_model->updated($invoice_id);
    if($check){
         $this->session->set_userdata('exception','Update successfully');
    }else{
       $this->session->set_userdata('exception','Submission Failed');
    }
    redirect("canteen/Invoicerec/lists");
  }

  function received($invoice_id=FALSE){
      $check=$this->Invoicerec_model->received($invoice_id);
      if($check){ 
