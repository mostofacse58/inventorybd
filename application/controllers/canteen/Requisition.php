...
class Requisition extends My_Controller {
    ...
    function lists(){
      ...
      $data['dlist']=$this->Look_up_model->getSupplier();
...
public function suggestions(){
        ...
        $rows = $this->Requisition_model->getRequisitionProduct($term);
        if ($rows){
            $c = str_replace(".", "", microtime(true));
            $r = 0;
            foreach ($rows as $row) {
              $product_name="$row->product_name($row->product_code)";
                $pr[] = array('id' => ($c + $r), 'product_id' => $row->product_id, 'label' => $row->product_name . " (" . $row->product_code . ")", 'category_name' => $row->category_name ,'unit_name' => $row->unit_name,'product_name' =>$product_name ,'product_code' => $row->product_code, 'unit_price' => $row->unit_price,'image_link' => $row->product_image);
                $r++;
            }
    ...
    function view2($requisition_id=FALSE){
      ...
      $data['info']=$this->Requisition_model->get_info($requisition_id);
      $data['detail']=$this->Requisition_model->getDetails($requisition_id);
      $data['display']='canteen/requisitionView2';
      $this->load->view('admin/master',$data);
    }
    ...
    if ($this->session->userdata('user_id')) {
        $data['heading']='Requisition Form';
            ...
            $html=$this->load->view('canteen/requisitionView', $data, true);
