...
class Sreturn extends My_Controller {
    function __construct(){
        parent::__construct();
        $this->load->model('canteen/Sreturn_model');
    ...
    function lists(){
      $data=array();
        ...
        $config['base_url']=base_url().'canteen/Sreturn/lists/';
        ...
        $data['display']='canteen/sreturnlist';
    ...
    function add(){
      ...
      $data['slist']=$this->Look_up_model->getSupplier();
      $data['display']='canteen/addsreturn';
      $this->load->view('admin/master',$data);
    }
          ...
          foreach ($product_id as $value) {
           ...
           // echo "$fifoStock  $preReturnQty $main_stock"; exit();
            if($quantity[$i]>($fifoStock+$preReturnQty)&&$quantity[$i]>($main_stock+$preReturnQty)) {
              ...
              if($sreturn_id==FALSE)
                redirect("canteen/Sreturn/add");  
              else 
                redirect("canteen/Sreturn/edit/$sreturn_id"); 
            }
            $i++;
          }
        /////////////////////////////////
    ...
    function delete($sreturn_id=FALSE){
      ...
      redirect("canteen/Sreturn/lists");
    }
  ////////////////////////
  public function suggestions(){
    ...
    if ($this->session->userdata('user_id')) {
        $data['heading']='Invoice Spares ';
            ...
            $html=$this->load->view('canteen/viewMaterialUsing', $data, true);
