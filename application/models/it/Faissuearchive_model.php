<?php
class Faissuearchive_model extends CI_Model {
  public function get_count(){
    ...
    if($_POST){
      ...
      $this->session->set_userdata('assetstatus',$condition);
     }
    $this->session->unset_userdata('assetstatus');
    $department_id=$this->session->userdata('department_id');
      $query=$this->db->query("SELECT sim.* FROM asset_issue_master sim
        ...
        WHERE sim.issue_status!=1 AND sim.department_id=$department_id $condition");
    ...
    $department_id=$this->session->userdata('department_id');
        if($asset_issue_id==FALSE){
           $result=$this->db->query("SELECT pd.product_detail_id,
            ...
            WHERE pd.product_id=p.product_id AND pd.detail_status=1 AND 
            ...
            CONCAT(p.product_name,' (',pd.asset_encoding,')') as product_name
            FROM product_detail_info pd, product_info p
            WHERE pd.product_id=p.product_id AND pd.detail_status=1  AND pd.department_id=$department_id
            AND pd.product_detail_id NOT IN(SELECT sim.product_detail_id 
            FROM asset_issue_master sim 
   ...
   function getUNUSEDMachineList($term){
    $data=date('Y-m-d');
     $result=$this->db->query("SELECT pd.product_detail_id,
            ...
            AND pd.product_detail_id NOT IN(SELECT sim.product_detail_id 
            FROM asset_issue_master sim 
            WHERE sim.take_over_status=1) AND (pd.ventura_code LIKE '%$term%' or pd.asset_encoding LIKE '%$term%' or p.product_name LIKE '%$term%') ")->result();
        return $result;
    }
    ...
    function underService(){
        ...
        $product_detail_id=$this->db->query("SELECT product_detail_id FROM asset_issue_master 
          ...
          $this->db->WHERE('product_detail_id',$product_detail_id);
          $this->db->update('product_detail_info',array('it_status'=>3));
        /////////////////////////////////////////////
      return $query;
    ...
    function returndate(){
        ...
        $product_detail_id=$this->db->query("SELECT product_detail_id FROM asset_issue_master 
          WHERE asset_issue_id=$asset_issue_id")->row('product_detail_id');
          $this->db->WHERE('product_detail_id',$product_detail_id);
          $this->db->update('product_detail_info',array('it_status'=>2));
        /////////////////////////////////////////////
      return $query;
