<?php
class Underservice_model extends CI_Model {
  public function get_count(){
    ...
    $department_id=$this->session->userdata('department_id');
      $query=$this->db->query("SELECT sim.* FROM asset_issue_master sim
        ...
        WHERE pd.it_status=3 AND sim.issue_status=3 
        AND sim.department_id=$department_id $condition");
  ...
  function lists($limit,$start) {
    ...
    if($_GET){
     ...
     $result=$this->db->query("SELECT sim.*,pd.*,p.product_name,p.product_code,
      ...
      INNER JOIN asset_issue_master sim ON(sim.product_detail_id=pd.product_detail_id AND sim.issue_status=3)
      INNER JOIN product_info p ON(p.product_id=pd.product_id)
      ...
      WHERE pd.it_status=3 AND sim.department_id=$department_id $condition
