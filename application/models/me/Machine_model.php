...
class Machine_model extends CI_Model {
    function lists() {
        ...
        $result=$this->db->query("SELECT p.*,c.category_name,m.mtype_name,
          ...
          WHERE p.department_id=12 and p.product_type=1 AND p.machine_other=1
    ...
    function get_info($product_id){
        $result=$this->db->query("SELECT p.*,c.category_name,m.mtype_name,
          ...
          LEFT JOIN brand_info b ON(p.brand_id=b.brand_id)
          LEFT JOIN user u ON(u.id=p.user_id) 
          WHERE p.department_id=12 and p.product_type=1 AND 
          p.machine_other=1 AND p.product_id=$product_id")->row();
        return $result;
    }
