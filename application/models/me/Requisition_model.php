...
class Requisition_model extends CI_Model {
  public function get_count(){
    ...
    $department_id=$this->session->userdata('department_id');
      $data=$this->db->query("SELECT count(*) as counts     
          FROM  requisition_master pm 
                   WHERE pm.pr_type=1 AND pm.general_or_tpm=2
                    AND pm.user_id=$user_id
          $condition")->row('counts');
      $data=$this->db->query("SELECT count(*) as counts      
          FROM  requisition_master pm 
          WHERE pm.pr_type=1 AND pm.general_or_tpm=2 
          AND pm.user_id=$user_id $condition")->row('counts');
      return $data;
    }
    ...
    function lists($limit,$start) {
        ...
        $result=$this->db->query("SELECT pm.*,pt.department_name,u.user_name,
          ...
          WHERE pm.general_or_tpm=2 AND pm.user_id=$user_id $condition
          ORDER BY pm.requisition_status ASC,pm.requisition_id DESC LIMIT $start,$limit")->result();
        return $result;
    }
    function get_info($requisition_id){
      $department_id=$this->session->userdata('department_id');
         $result=$this->db->query("SELECT pm.*,pt.department_name,
          ...
          WHERE pm.requisition_id=$requisition_id AND pm.general_or_tpm=2")->row();
        return $result;
    }
    function save($requisition_id) {
      $department_id=$this->session->userdata('department_id');
      $data=array();
      $data['responsible_department']=12;
 ...
 function getRequisitionProduct($term,$asset_encoding) {
    $data=date('Y-m-d');
