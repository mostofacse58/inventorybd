...
class Stockout_model extends CI_Model {
  public function get_count(){
    $condition=' ';
      if($_GET){
        if($this->input->get('status')!='All'){
          ...
          $condition=$condition."  AND p.out_status='$status' ";
        }
    ...
    $department_id=$this->session->userdata('department_id');
      $query=$this->db->query("SELECT * FROM stock_out_master p
        WHERE 1  $condition");
    ...
    function waiting(){
      $result=$this->db->query("SELECT count(id) as waiting
        FROM  stock_out_master p
        WHERE out_status=1")->row('waiting');
      return $result;
    }
    function total(){
    ...
    function alreadyIn(){
      $result=$this->db->query("SELECT count(id) as alreadyIn
        FROM  stock_out_master p
        WHERE out_status=2 ")->row('alreadyIn');
      return $result;
    }
    function waitingInv($export_invoice_no){
      $result=$this->db->query("SELECT count(id) as waiting
        FROM  stock_out_master p
        WHERE out_status=1 AND export_invoice_no='$export_invoice_no' ")->row('waiting');
      return $result;
    }
    function alreadyInv($export_invoice_no){
      $result=$this->db->query("SELECT count(id) as alreadyIn
        FROM  stock_out_master p
        WHERE out_status=2  AND export_invoice_no='$export_invoice_no' ")->row('alreadyIn');
      return $result;
    }
    function getGroupByInvoice(){
      $result=$this->db->query("SELECT p.export_invoice_no,p.out_status,
        (SELECT count(s.id) as pendingqty FROM stock_out_master s 
        WHERE s.export_invoice_no=p.export_invoice_no AND s.out_status=1) as pendingqty,
        (SELECT count(s1.id) as inqty FROM stock_out_master s1 
        WHERE s1.export_invoice_no=p.export_invoice_no AND s1.out_status=2) as inqty
        FROM  stock_out_master p
        WHERE 1 
    ...
    function getGroupByPo($export_invoice_no){
      $result=$this->db->query("SELECT p.po_no,p.out_status,
        (SELECT count(s.id) as pendingqty FROM stock_out_master s 
        WHERE s.po_no=p.po_no AND s.out_status=1 AND s.export_invoice_no='$export_invoice_no') as pendingqty,
        (SELECT count(s1.id) as inqty FROM stock_out_master s1 
        WHERE s1.po_no=p.po_no AND s1.out_status=2 AND s1.export_invoice_no='$export_invoice_no') as inqty
        FROM  stock_out_master p
    ...
    function save($id) {
        ...
        $data['out_document_no']=$this->input->post('out_document_no');
        ...
        $query=$this->db->insert('stock_out_master',$data);
          ...
          $this->db->WHERE('id',$id);
          $query=$this->db->update('stock_out_master',$data);
        }
      return $id;
    }
    ...
    function delete($id) {
        $this->db->WHERE('id',$id);
        $query=$this->db->delete('stock_out_master');
        return $query;
    }
