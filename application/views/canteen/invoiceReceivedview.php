...
<script type="text/javascript">
    function calculateRow(id){
        ...
        var actualquantity=$("#actualquantity_"+id).val();
        if($.trim(actualquantity)==""|| $.isNumeric(actualquantity)==false){
            actualquantity=0;
        }
        if($.trim(unit_price)==""|| $.isNumeric(unit_price)==false){
            unit_price=0;
        }
        var quantityAndPrice=parseFloat($.trim(unit_price))*parseFloat($.trim(actualquantity));
        $("#actualamount_"+id).val(quantityAndPrice.toFixed(2));
    }
...
<h5><?php echo ucwords($heading); ?></h5>
  ...
  <!-- /.box-header -->
  <form class="form-horizontal" action="<?php echo base_url(); ?>canteen/Invoicerec/updated<?php if (isset($info)) echo "/$info->invoice_id"; ?>" method="POST" enctype="multipart/form-data" onsubmit="return formsubmit();">
    <div class="box-body">
    ...
    <table class="tg">
      <tr>
        ...
        <th style="width:10%;text-align:center;">Actual Qty</th>
        <th style="width:10%;text-align:center;">Actual Amount</th>
      ...
      <tr>
        ...
        <td class="tg-s6z2">
          ...
          <input type="text" name="actualquantity[]" onfocus="this.select();" value="<?php echo $value->actualquantity; ?>" onblur="return calculateRow(<?php echo $m; ?>);" onkeyup="return calculateRow(<?php echo $m; ?>);" required class="form-control"  placeholder="actualquantity" style="width:100%;float:left;text-align:center"  id="actualquantity_<?php echo $m; ?>"> 
        </td>
        <td class="tg-s6z2">
          <input type="text" name="actualamount[]" value="<?php echo $value->actualamount; ?>" readonly class="form-control"  placeholder="actualamount" style="width:100%;float:left;text-align:center"  id="actualamount_<?php echo $m; ?>"> 
        </td>
