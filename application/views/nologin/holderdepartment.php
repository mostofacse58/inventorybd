...
<!-- <div class="container">
  ...
  <thead>
              ...
              ?>
      <?php $info=$this->db->query("SELECT d.*,0 as totalreq,
        (SELECT IFNULL(SUM(id.quantity),0) as dd FROM 
        spares_use_detail id 
        WHERE id.department_id=d.department_id) as totalqty,
        (SELECT IFNULL(SUM(id1.quantity*p.unit_price),0) as dd FROM spares_use_detail id1,product_info p
        WHERE id1.department_id=d.department_id 
        AND id1.product_id=p.product_id) as totalamount
        FROM department_info d 
        WHERE  d.department_id=12")->row(); ?>
        <tr>
          <td style="text-align:center;">
            <?php echo $info->department_name;  ?></td>  
          <td style="text-align:center;">
            <?php echo $info->totalreq;  ?></td>  
            <td style="text-align:center;">
            <?php echo (int) $info->totalqty;  ?></td>  
            <td style="text-align:center;">
            <?php echo (int) $info->totalamount;  ?></td>  
        </tr>
              </tbody>
