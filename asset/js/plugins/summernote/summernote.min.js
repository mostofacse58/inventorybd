...
return{
    ...
    var d=n(b,f.eq(a));
    return 1===d.length?A(b,c):d.reduce(function(a,d){
        var e=d.cloneNode(!1);
...
return c.moveToElementText(d.cont),c.collapse(d.collapseToStart),c.moveStart("character",d.offset),c
    ...
    this.sc=c,this.so=e,this.ec=i,this.eo=j;
    var k=function(){
        if(b){
            var a=document.createRange();
            return a.setStart(c,e),a.setEnd(i,j),a
            }
            var f=d({
            cont:c,
            offset:e
        });
        return f.setEndPoint("EndToEnd",d({
            cont:i,
            offset:j
        })),f
        };
    ...
    this.select=function(){
        ...
        var d=h.listBetween(c,i),e=g.compact(a.map(d,function(a){
            return h.ancestor(a,b)
            }));
        return a.map(g.clusterBy(e,f.eq2),g.head)
        },this.commonAncestor=function(){
        return h.commonAncestor(c,i)
        };
        
    var l=function(a){
        return function(){
            var b=h.ancestor(c,a);
            return!!b&&b===h.ancestor(i,a)
            }
        };
    
this.isOnEditable=l(h.isEditable),this.isOnList=l(h.isList),this.isOnAnchor=l(h.isAnchor),this.isOnCell=l(h.isCell),this.isCollapsed=function(){
    return c===i&&e===j
...
return{
    ...
    for(var d=this.recordUndo=function(a){
        ...
        },f=["bold","italic","underline","strikethrough","superscript","subscript","justifyLeft","justifyCenter","justifyRight","justifyFull","insertOrderedList","insertUnorderedList","indent","outdent","formatBlock","removeFormat","backColor","foreColor","insertHorizontalRule","fontName"],g=0,i=f.length;i>g;g++)this[f[g]]=function(a){
        return function(b,c){
...
this.tab=function(a,b){
    var d=m.create();
    d.isCollapsed()&&d.isOnCell()?c.tab(d):k(a,d,b.tabsize)
    },this.untab=function(){
    var a=m.create();
...
for(var g=1;6>=g;g++)this["formatH"+g]=function(a){
    return function(b){
...
this.fontSize=function(a,b){
    ...
    },this.createLink=function(b,c,f){
    var g=c.url,i=c.text,j=c.newWindow,k=m.create();
    if(d(b),f.onCreateLink&&(g=f.onCreateLink(g)),!k.isOnAnchor())if((e.isMSIE||e.isFF)&&k.isCollapsed()){
        k.insertNode(a('<A id="linkAnchor">'+i+"</A>")[0]);
        var l=a("#linkAnchor").attr("href",g).removeAttr("id");
        k=m.createFromNode(l[0]),k.select()
        }else document.execCommand("createlink",!1,g);
    a.each(k.nodes(h.isAnchor),function(b,c){
        a(c).html(i),j?a(c).attr("target","_blank"):a(c).removeAttr("target")
        })
    },this.getLinkInfo=function(b){
    b.focus();
    var c=m.create(),d=!0,e="";
    if(c.isOnAnchor()){
        var f=h.ancestor(c.sc,h.isAnchor);
        c=m.createFromNode(f),d="_blank"===a(f).attr("target"),e=f.href
        }
        return{
        text:c.toString(),
        url:e,
...
this.update=function(a,h,i){
    b.update(a,h);
    var j=a.find(".note-link-popover");
    if(h.anchor){
        var k=j.find("a");
        k.attr("href",h.anchor.href).html(h.anchor.href),d(j,c(h.anchor,i))
        }else j.hide();
    var l=a.find(".note-image-popover");
    h.image?d(l,c(h.image,i)):l.hide();
    var m=a.find(".note-air-popover");
    if(i&&!h.range.isCollapsed()){
        var n=f.rect2bnd(g.last(h.range.getClientRects()));
        d(m,{
            left:Math.max(n.left+n.width/2-e,0),
            top:n.top+n.height
            })
        }else m.hide()
